...
def esm2_pipeline(checkpoint,
    ...
    # ---------------- Download already fine-tuned models from S3 ---------------

    if mode != 'base':

        s3_prefix_2 = f'results_2/{pathogen_name}/{checkpoint_name}_{mode}_fine_tuned_model/'

        model_dir = local_output / f'{checkpoint_name}_{mode}_fine_tuned_model'

        response = s3.list_objects_v2(Bucket=bucket, Prefix=s3_prefix_2)
        ...
        for obj in response.get('Contents', []):
            ...
            relative_path = key[len(s3_prefix_2):]
