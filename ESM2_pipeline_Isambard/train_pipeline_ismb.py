...
from utils_ismb import (load_esm_model_classification, select_datasets, preprocess_csv,
...
def esm2_pipeline(checkpoint,
    ...
    local_output = Path('./HB/output') / pathogen_name / f'{checkpoint_name}_{mode}'
    ...
    parameter_summary.to_csv(Path(f'{local_output}_trainable_parameters_summary.csv'))
    ...
    for fold in val_loaded.keys():
        ...
        rows.append(pd.DataFrame(
            {'Fold': fold,
             ...
             'Prob': probs,
             'Preds': preds}))
    pd.concat(rows, ignore_index=True).to_csv(Path(f'{local_output}_clf_cv_roc_epoch.csv'), index=False)
